<template>
  <div v-if="variableData.length > 0">
    <h4>Harmonizations of {{ variable }}</h4>
    <div class="table-responsive">
      <table class="table table-sm" cellspacing="0">
        <thead>
        <tr>
          <th></th>
          <th v-for="variable in variableData"
              @click="selectedVariable = variable;showCoreVariable = !showCoreVariable">{{ variable.variable }}
          </th>
        </tr>
        ...
        <tr>
          ...
          <td v-for="variable in variableData">
            <div v-if="variableExists(cohort.id, variable.harmonizations)" style="font-size:20px">
              <router-link :to="'/' + variable.variable + '/' + getSelectedHarmonization(variable, cohort.id)"><i
                class="fa fa-check-circle text-success"></i></router-link>
            </div>
